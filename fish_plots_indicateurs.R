...
## Load data
reads <- read.csv("teleo_reads_oct21_repassee.csv", header=T, row.names=1) %>%
  replace(is.na(.), 0) %>%
  filter(rowSums(.) > 0) %>%
  t(.)  %>%
  as.data.frame(.)
...
traits <- read.csv("Functional_data_corrected_20220124.csv", header=T)

###################################################### une fois créé à mettre en commentaire
## créer la matrice de présence/absence à partir des reads
data <- reads %>%
  t(.) %>%
  as.data.frame(.) %>%
  mutate_if(is.numeric, ~1 * (. != 0)) 
write.csv(data, file="teleo_presence_oct21_repassee.csv")
...
write.csv(data2, file="teleo_presence_oct21_repassee.csv")
...
write.csv(indicators, file="indicators_ports_oct21.csv")
...
# Charger datas ports
meta <- read.csv("metadata_eDNA_port_october_2021.csv", header=T)
ind_ports <- read.csv("indicators_ports_oct21_repassee.csv", header=T, row.names=1) %>%
...
## Save plot
png("Boxplots_indicators_port_milieu_nat3.png", 
