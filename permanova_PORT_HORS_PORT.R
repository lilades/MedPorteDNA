...
adne <- read.csv("matrice_adne_totale_repassee.csv", header=T, row.names=1)
# Enlever les pts biohuts
adne <- adne[,-98]
adne <- adne[,-93]
adne <- adne[,-86]
...
## Perform PERMANOVA : tester la significativité de la variable "habitat" (= port ou hors port)
...
ggsave("PCoA_horsport_port.png", width = 11, height = 8)
