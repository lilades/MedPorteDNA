...
###COMPARAISON PORT PROPRE VS NON CERTIFIÉ 

# Metadata
meta <- read.csv("metadata_port.csv", header=T)
# Enlever les biohuts
meta <- meta[,-1]
meta <- meta[-11,]
meta <- meta[-8,]
...
# Enlever les ports avec biohuts
adne <- adne [,-15]
adne <- adne [,-10]
adne <- adne [,-3]
...
## Perform PERMANOVA : tester la significativité de la variable "propreté"
...
grp2.hull<-seg.data[seg.data$group=="Non certifié",1:3][chull(seg.data[seg.data$group=="Non certifié",2:3]),]
...
panel.a<-ggplot() + 
  ...
  theme_mine + 
  scale_colour_manual(values=c("lightblue","coral"), labels = c("Non certifié", "Certifié Ports propres")) +
  scale_shape_manual(values=c(19,17), labels = c("Non certifié", "Certifié Ports propres")) +
  labs(title="",x="PCoA 1",y="PCoA 2",
       colour = "Certification du port", shape = "Certification du port")

panel.a

ggsave("PCoA_port_propre.png", width = 11, height = 8)
