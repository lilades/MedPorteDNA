...
## Load data
data <- read.csv("biodiv_milieu_naturel.csv", header=T, row.names=1) %>%
  t(.) %>%
  as.data.frame(.) 
...
meta <- read.csv("metadata_milieu_naturel.csv", header=T)
...
write.csv(sp_hors_port_tot_traits, "sp_hors_port_tot_traits.csv")
